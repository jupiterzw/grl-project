...
parser.add_argument('--flag_step_size', type=float, default=0.005, help='Step size for FLAG perturbations')
parser.add_argument('--flag_ascent_steps', type=int, default=3, help='Number of ascent steps for FLAG')
...
for run in range(args.runs):
    ...
    for epoch in range(args.epochs):
        ...
        if args.method == 'flag':
            perturbation = torch.FloatTensor(dataset_tr.graph['node_feat'].shape).uniform_(-args.flag_step_size, args.flag_step_size).to(device)
            perturbation.requires_grad_()

            for _ in range(args.flag_ascent_steps):
                optimizer.zero_grad()
                loss = model(dataset_tr, criterion, perturbation)
                loss.backward()
                perturbation.data = perturbation.data + args.flag_step_size * perturbation.grad.sign()
